...
* Version            : V1.1x
* Date               : 2019/10/21
* Description        : CH549 USB 主机接口函数, add CH545/CH557
*******************************************************************************/
...
// #include ".\USB\USB_LIB\CH549UFI.H"                                         //如果使用USBH_HUB_KM.C 此行屏蔽掉
...
* Description    : CH549传输事务,输入目的端点地址/PID令牌,同步标志,以20uS为单位的NAK重试总时间(0则不重试,0xFFFF无限重试),返回0成功,超时/出错重试
...
UINT8 InitDevOnHub( UINT8 HubPortIndex ) 
{
	...
	DevOnHubPort[HubPortIndex-1].DeviceVID = (((UINT16)((PXUSB_DEV_DESCR)Com_Buffer)->idVendorH)<<8 ) + ((PXUSB_DEV_DESCR)Com_Buffer)->idVendorL; //保存VID PID信息
...
{
	UINT8  RootHubIndex;                                                          //CH549只有一个USB口,RootHubIndex = 0,只需看返回值的低八位即可
    UINT8  HubPortIndex;
