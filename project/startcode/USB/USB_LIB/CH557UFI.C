...
**   Web:      http://wch.cn           **
*****************************************
**  USB-flash File Interface for CH557@CH549LIB **
**  KC7.0@MCS51                        **
*****************************************
*/
...
UINT8	CH549DiskConnect( void )
{
	...
	if ( rhub_idx1 < 4 ) {  /* 内置Root-HUB下的USB设备 */
		if ( ( rhub_idx1 & 0x02 ? UHUB23_CTRL : UHUB01_CTRL ) & ( rhub_idx1 & 0x01 ? bUH1_PORT_EN : bUH0_PORT_EN ) ) {  /* 内置Root-HUB下的USB设备存在且未插拔 */
			return( ERR_SUCCESS );  /* USB设备已经连接且未插拔 */
		}
		else if ( USB_HUB_ST & (UINT8)( (UINT8)bUHS_H0_ATTACH << rhub_idx1 ) ) {  /* USB设备存在 */
mDiskConnect:
...
mDiskDisconn:
	...
	else if ( rhub_idx2 < 4 && (UINT8)( ( USB_DEV_AD & 0x0F ) - 1 ) < 4 ) {  /* 外部HUB的端口下的USB设备 */
		if ( ( rhub_idx2 & 0x02 ? UHUB23_CTRL : UHUB01_CTRL ) & ( rhub_idx2 & 0x01 ? bUH1_PORT_EN : bUH0_PORT_EN ) ) {  /* 内置Root-HUB下的外部HUB存在且未插拔 */
			TxBuffer[ MAX_PACKET_SIZE - 1 ] = USB_DEV_AD;  /* 备份 */
			USB_DEV_AD = USB_DEVICE_ADDR + rhub_idx2;  /* 设置USB主机端的USB地址指向HUB */
...
//					return( ERR_USB_DISCON );
		...
		else if ( USB_HUB_ST & (UINT8)( (UINT8)bUHS_H0_ATTACH << rhub_idx2 ) ) {  /* USB设备存在,外部HUB或USB设备已经连接或者断开后重新连接 */
//			CH549DiskStatus = DISK_CONNECT;  /* 曾经断开过 */
//			return( ERR_SUCCESS );  /* 外部HUB或USB设备已经连接或者断开后重新连接 */
			goto mDiskConnect;
		}
