...
* Date               : 2018/08/15
* Description        :
 CH549 C语言的U盘文件字节读写示例程序，文件指针偏移，修改文件属性，删除文件等操作
    ...
    while ( 1 )
    {
        ...
        if ( FoundNewDev || s == ERR_USB_CONNECT )                         // 有新的USB设备插入
        {
            ...
            if ( s == ERR_SUCCESS )
            {
                ...
                {
//                  /* 读文件 */
//                  strcpy( mCmdParam.Open.mPathName, "/C51/CH549HFT.C" );     //设置将要操作的文件路径和文件名/C51/CH549HFT.C
...
//                      printf( "读出的前%d个字符是:\n",TotalCount );
//                      while ( TotalCount ) {                                 //如果文件比较大,一次读不完,可以再调用CH549ByteRead继续读取,文件指针自动向后移动
...
//                           s = CH549ByteRead( );                                /* 以字节为单位读取数据块,单次读写的长度不能超过MAX_BYTE_IO,第二次调用时接着刚才的向后读 */
...
//                  mCmdParam.ByteRead.mByteBuffer= &buf[0];
//                  CH549ByteRead( );   //直接读取文件的第608个字节到612个字节数据,前608个字节被跳过
...
//                  CH549ByteWrite( );   //在原文件的后面添加数据,新加的13个字节接着原文件的尾部放置
//                  mCmdParam.ByteWrite.mByteCount = 2;  //写入2个字节的数据
//                  CH549ByteWrite( );   //继续在原文件的后面添加数据
//                  mCmdParam.ByteWrite.mByteCount = 0;  //写入0个字节的数据,实际上该操作用于通知程序库更新文件长度
//                  CH549ByteWrite( );   //写入0字节的数据,用于自动更新文件的长度,所以文件长度增加15,如果不这样做,那么执行CH554FileClose时也会自动更新文件长度
                    //创建文件演示
                    printf( "Create\n" );
...
//                  i = CH549FileModify( );   //修改当前文件的信息,修改日期
//                  mStopIfError( i );
                    printf( "Close\n" );
